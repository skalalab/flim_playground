...
## Milestone 3 – Config Era (Jul 2025 – Jul 2026)
...
- `f7abe0d` (2026-06-30) completed the symmetry by making extraction configs profile-based too, so configuration became a portfolio of independent setups rather than one live setting—`_load_active_profile_cfg()` resolves whichever profile is active transparently, letting every accessor and downstream consumer keep reading "the config" unchanged.
- `5247095` (2026-07-03) closed the era by turning configuration back on its own output: features became *composable* rather than only measured, as a restricted-AST mini-language builds cross-channel columns like `A / (A + B)` that the analysis layer consolidates into a single `Derived:` group, while `src/feature_schema.py` *predicts* the columns a profile will emit so those formulas can be authored before any extraction has ever run. Configuration stopped merely describing the data and started reasoning about the data it was about to create.
