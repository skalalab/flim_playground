...
streamlit-desktop-app build main.py --name Flim-Playground  --icon logo.png --pyinstaller-options --add-data src:src --add-data pages:pages --add-data config.toml:. --hidden-import pages.classification --hidden-import pages.visualization --hidden-import pages.data_extraction --collect-all streamlit_plotly_events --onefile --noconfirm --clean
