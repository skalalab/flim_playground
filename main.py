...
def main():
    ...
    if imaging_modality == "FLIM" and flim_decay_input_type == "Decay (2D)":
        ...
        with cols[1]:
            cfg[flim_decay_input_type]["time_bins"] = st.number_input(f"{flim_decay_input_type} time bins", value=cfg.get(flim_decay_input_type, {}).get("time_bins", 1024), min_value=10, key=f"{flim_decay_input_type}_time_bins")
