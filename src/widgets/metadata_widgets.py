...
def load_list_data_from_folder_widget(folder_path, file_suffix, num_cols=3):    
    ...
    all_files = [str(file) for file in path.rglob("*") if file.is_file() and not file.name.startswith("fov_metadata") and not file.name.startswith("single_cell_featuress")]
